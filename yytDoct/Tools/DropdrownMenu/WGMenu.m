...
typedef enum {
    ...
    WGMenuViewArrowDirectionNone,
    WGMenuViewArrowDirectionUp,
    WGMenuViewArrowDirectionDown,
    WGMenuViewArrowDirectionLeft,
    WGMenuViewArrowDirectionRight,
    
} WGMenuViewArrowDirection;


@interface WGMenuView : UIView
...
@interface WGMenuOverlay : UIView
...
#pragma mark - WGMenuOverlay

@implementation WGMenuOverlay
...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[WGMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]) {
...
#pragma mark - WGMenuView
@implementation WGMenuView {
    WGMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView *_contentView;
    NSArray *_menuItems;
    WGMenuSelectedItem _selectedItem;
}
...
- (id)init {
    ...
    if(self) {
        ...
        if ([WGMenu hasShadow]) {
           ...
           self.layer.shadowColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.5].CGColor;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect {
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) {
        ...
        _arrowDirection = WGMenuViewArrowDirectionUp;
        ...
        _arrowDirection = WGMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = WGMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
        ...
        _arrowDirection = WGMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
        ...
        _arrowDirection = WGMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(WGMenuSelectedItem)selectedItem {
    ...
    WGMenuOverlay *overlay = [[WGMenuOverlay alloc] initWithFrame:view.bounds];
...
- (void)dismissMenu:(BOOL)animated {
    if (self.superview) {
        if (animated) {
            ...
            [UIView animateWithDuration:0.2 animations:^(void) {
                ...
                if ([self.superview isKindOfClass:[WGMenuOverlay class]])
    ...
    [WGMenu sharedMenu].isShow = NO;
}

- (void)performAction:(id)sender {
    [self dismissMenu:YES];
    UIButton *button = (UIButton *)sender;
    WGMenuItem *menuItem = _menuItems[button.tag];
...
//     const CGFloat kMarginY = 5.f;
    const CGFloat kMarginY = 14.5f;
    
    UIFont *titleFont = [WGMenu titleFont];
    
//    CGFloat maxImageWidth = 0;
    
    CGFloat maxImageWidth = 0 + 60 ;
    
    CGFloat maxItemHeight = 0;
    CGFloat maxItemWidth = 0;
    
    for (WGMenuItem *menuItem in _menuItems) {
    ...
    UIImage *selectedImage = [WGMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
//    UIImage *gradientLine = [WGMenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
    
    UIImage * gradientLine = [WGMenuView gradientLine:(CGSize) {maxItemWidth , 0.5}];
...
//        const CGRect itemFrame = (CGRect){0, itemY, maxItemWidth, maxItemHeight};
        const CGRect itemFrame = (CGRect){0, itemY-28.7, maxItemWidth, 50};
...
//            const CGRect imageFrame = {kMarginX * 2, kMarginY, maxImageWidth, maxItemHeight - kMarginY * 2};
            ...
            const CGRect imageFrame = {19 , 25 - 16/2 , 16 , 16}; //固定值
...
//            gradientView.frame = (CGRect){kMarginX * 2, maxItemHeight + 1, gradientLine.size};
            ...
            gradientView.backgroundColor = [UIColor colorWithRed:204/255.0 green:204/255.0 blue:204/255.0 alpha:0.5f ];
    ...
    contentView.frame = (CGRect){0, 0, maxItemWidth, 50 * _menuItems.count};
//    contentView.frame = (CGRect){0 , 0 , maxItemWidth , 100};
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == WGMenuViewArrowDirectionUp) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };
        
    } else if (_arrowDirection == WGMenuViewArrowDirectionDown) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };
        
    } else if (_arrowDirection == WGMenuViewArrowDirectionLeft) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };
        
    } else if (_arrowDirection == WGMenuViewArrowDirectionRight) {
...
//    }
    ...
    gSelectedColor = [UIColor colorWithRed:224/255.0 green:(224/255.0) blue:224/255.0 alpha:1.0f];
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    UIColor *tintColor = [WGMenu tintColor];
    if (tintColor) {
        CGFloat a;
    ...
    if ([WGMenu backgrounColorEffect] == WGMenuBackgrounColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == WGMenuViewArrowDirectionLeft ||
        _arrowDirection == WGMenuViewArrowDirectionRight) {
...
/// 背景色效果
static WGMenuBackgrounColorEffect   gBackgroundColorEffect = WGMenuBackgrounColorEffectSolid;
/// 是否显示阴影
...
+ (void)reset {
    gTintColor = nil;
    gTitleFont = nil;
    gBackgroundColorEffect = WGMenuBackgrounColorEffectSolid;
    gHasShadow = NO;
    gSelectedColor = [UIColor colorWithRed:0.059 green:0.353 blue:0.839 alpha:1.0f];
}
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(WGMenuSelectedItem)selectedItem {
    ...
    // 创建MenuView
    _menuView = [[WGMenuView alloc] init];
